...
*    fn: The camera frame number
*    width: Width of the camera frame buffer
*    height:Height of the camera frame buffer
...
//    uint32_t exposure;
//    uint8_t* buffer; //xiaoyang  volatile uint8_t* buffer
//    uint32_t len;
//    uint32_t width;
//    uint32_t height;
...
//} qvrcamera_frame_t;
//-------------------------------------xiaoyang,xiugai,4.0.4,for get camera data
typedef struct qvrcamera_frame
{
...
*   QVRCameraClient_GetFrame.
*
*   QVRCAMERA_MODE_BLOCKING
*   QVRCameraClient_GetFrame will block until the requested frame number
*   is available.
*   If the requested frame number is available, QVRCameraClient_GetFrame
*   returns immediately.
*
*   QVRCAMERA_MODE_NON_BLOCKING
*   QVRCameraClient_GetFrame will not block.
*   If the requested frame number is not available, QVRCameraClient_GetFrame
*   returns immediately with return code QVR_CAM_FUTURE_FRAMENUMBER.
******************************************************************************/
typedef enum {
  QVRCAMERA_MODE_BLOCKING,
  QVRCAMERA_MODE_NON_BLOCKING
...
*   QVRCameraClient_GetFrame will attempt to return at least the
*   requested frame number or a newer if available.
*   If frame number is in the future, QVRCameraClient_GetFrame either blocks
*   or returns with error code QVR_CAM_FUTURE_FRAMENUMBER.
*
*   QVRCAMERA_MODE_EXPLICIT_FRAME_NUMBER
*   QVRCameraClient_GetFrame will attempt to return the requested frame number.
*   If frame number is too old, QVRCameraClient_GetFrame returns with error code
*   QVR_CAM_EXPIRED_FRAMENUMBER.
*   If frame number is in the future, QVRCameraClient_GetFrame either blocks
*   or returns with error code QVR_CAM_FUTURE_FRAMENUMBER.
******************************************************************************/
typedef enum {
...
typedef struct qvrcamera_client_ops {
    ...
    * \param
    *    client:      [in] qvrcamera_client_handle_t returned by Create().
    *    camera_name: [in] name of the camera to open (QVRSERVICE_CAMERA_NAME_*)
    * \return
    ...
    *    CreateWithKey() with the same key. The key may be used defined, or it
    ...
    ***************************************************************************/
    int32_t (*RegisterForClientNotification)(qvrcamera_client_handle_t client,
            QVRCAMERA_CLIENT_NOTIFICATION notification, camera_client_notification_callback_fn cb,
            void *pCtx);
...
typedef struct qvrcamera_ops {
    ...
    * right after the next start of frame, and will take effect in a few frames
    ...
    * \return
    *    returns QVR_CAM_SUCCESS upon success, QVR_CAM_ERROR otherwise
    * This gamma correction settings is applied by the QVR camera Service
